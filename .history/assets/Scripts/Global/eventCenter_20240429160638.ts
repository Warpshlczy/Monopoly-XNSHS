    ...
    initMap: function (map: GameMap) {
      ...
      resources.load("prefabs/LandBlock", Prefab, (err, prefab) => {
        map.lands.forEach((landData) => {
          ...
          renamePrefab(newLand, `LandBlock_${newLand.data.game_id}`);
    ...
    initPlayers: function () {
      const initPiece = (player: Node, index: number) => {
        ...
        resources.load("prefabs/PlayerPiece", Prefab, (err, prefab) => {
          ...
          piece.$playerPiece.setAll(player, 0, 0);
          piece.$playerPiece.setPosition(
            gameMap[`LandBlock_${piece.$playerPiece.currentPosition}`].data.x,
            gameMap[`LandBlock_${piece.$playerPiece.currentPosition}`].data.y
          );
          this.allNodes.Canvas.GameMap.addChild(piece);
          this.updateNodes();
        });
      };
