...
export class GameManager extends Component {
  ...
  start() {
    ...
    const node = this.currentScene.addChild(new Node("SceneManager"));
  }
  update(deltaTime: number) {}
  loadSceneManager() {
    const manager = new Node("SceneManager");
    manager.addComponent(SceneManager);
    this.currentScene.addChild(manager);
  }
}
